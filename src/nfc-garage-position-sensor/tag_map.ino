const uint8_t TAGS[TAG_COUNT][TAG_UID_LENGTH] = {
  ...
  {0x04, 0xF5, 0x33, 0x16, 0xC9, 0x2A, 0x81},  // 0 %
  {0x04, 0x16, 0x34, 0x16, 0xC9, 0x2A, 0x81},  // 11 %
  {0x04, 0x15, 0x34, 0x16, 0xC9, 0x2A, 0x81},  // 22 %
  {0x04, 0x14, 0x34, 0x16, 0xC9, 0x2A, 0x81},  // 33 %
  {0x04, 0xFE, 0x33, 0x16, 0xC9, 0x2A, 0x81},  // 44 %
  {0x04, 0xFD, 0x33, 0x16, 0xC9, 0x2A, 0x81},  // 56 %
  {0x04, 0xFC, 0x33, 0x16, 0xC9, 0x2A, 0x81},  // 67 %
  {0x04, 0xFB, 0x33, 0x16, 0xC9, 0x2A, 0x81},  // 78 %
  {0x04, 0xF6, 0x33, 0x16, 0xC9, 0x2A, 0x81},  // 89 %
  {0x04, 0x17, 0x34, 0x16, 0xC9, 0x2A, 0x81}   // 100 %
};
...
uint8_t indexToPercent(int8_t index) {
  ...
  // Round to the nearest step so a 10-tag layout yields 0/11/22/.../89/100.
  uint8_t rawPercent = static_cast<uint8_t>(((index * 100) + ((TAG_COUNT - 1) / 2)) / (TAG_COUNT - 1));
